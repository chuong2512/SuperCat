...
public class IAPManager : PersistentSingleton<IAPManager>, IStoreListener
{
    ...
    private void InitProduct()
    {
        ...
        foreach (var field in fields)
        {
            ...
            Debug.Log($"product {fieldName} : {productKey} init");
            builder.AddProduct(productKey, GetProductType(fieldName));
        }

        UnityPurchasing.Initialize(this, builder);
    }

    private ProductType GetProductType(string iapKeyName)
    {
        var lower = iapKeyName.ToLower();
        if (lower.StartsWith("s")) return ProductType.Subscription;
        if (lower.StartsWith("nc")) return ProductType.NonConsumable;
